...
Во время заказа для стриминга траектории движения таксиста сообщения его местоположении передается в брокер сообщений Kafka, 
а оттуда их вычитывают фронтенд пассажира, а также сервис на серверной стороне, который складывает маршрут в поле "маршрут" таблицы поездок.

Сообщения в топике Kafka партиционированы ключом партиционирования, который содержит регион водителя.
...
Standalone Redis способен выдерживать 60k - 80k rps, поэтому 
